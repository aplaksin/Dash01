...
public class ZigzagEnemyBehaviour : IEnemyBeheviour
{
    ...
    public void GoExecute(float deltaTime)
    {
        if(_currentAction != _moveDown &&
            (Vector3.Distance(new Vector3(_transform.position.x, 0, 0), new Vector3(_minXPosition.x, 0, 0)) < 0.01f 
            || Vector3.Distance(new Vector3(_transform.position.x, 0, 0), new Vector3(_maxXPosition.x, 0, 0)) < 0.01f))
        {
        ...
        if(_currentAction != _moveVertical && (Vector3.Distance(new Vector3(0, _transform.position.y, 0), new Vector3(0, _lastYP - 2*_transform.localScale.y,0)) < 0.05f))
        {
            _currentAction = _moveVertical;
            _lastYP = -10f;
        }
