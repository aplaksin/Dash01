...
public class ZigzagEnemyBehaviour : IEnemyBeheviour
{
    ...
    {
        if(Vector3.Distance(_transform.position, _minXPosition) < 0.001f || Vector3.Distance(_transform.position, _maxXPosition) < 0.001f)
        {
            _currentAction = _moveDown;
            _moveVertical.SwapDirection();
        }
        else if(Vector3.Distance(_transform.position, _transform.position*_scaleVector) < 0.001f)
        {
            _currentAction = _moveVertical;
        }
