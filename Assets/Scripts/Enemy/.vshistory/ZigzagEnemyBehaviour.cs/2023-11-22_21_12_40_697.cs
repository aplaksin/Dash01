...
public class ZigzagEnemyBehaviour : IEnemyBeheviour
{
    ...
    public void GoExecute(float deltaTime)
    {
        if(_currentAction != _moveDown &&
            (Vector3.Distance(new Vector3(_transform.position.x, 0, 0), new Vector3(_minXPosition.x, 0, 0)) < 0.001f 
            || Vector3.Distance(new Vector3(_transform.position.x, 0, 0), new Vector3(_maxXPosition.x, 0, 0)) < 0.001f))
        {
        ...
        if(_currentAction != _moveVertical && (Vector3.Distance(new Vector3(0, _transform.position.y, 0), new Vector3(0, _lastYP - _transform.localScale.y,0)) < 0.001f))
        {
            _currentAction = _moveVertical;
            _lastYP = -10f;
        }
        ...
        if(_currentAction == _moveDown)
        {
            Debug.Log("-============================= " + Vector3.Distance(new Vector3(0, _transform.position.y, 0), new Vector3(0, _lastYP - _transform.localScale.y, 0)));
        }
