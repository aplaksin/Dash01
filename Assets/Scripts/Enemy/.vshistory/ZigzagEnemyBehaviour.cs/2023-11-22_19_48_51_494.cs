...
public class ZigzagEnemyBehaviour : IEnemyBeheviour
{
    ...
    {
        if(Vector3.Distance(new Vector3(0, _transform.position.y, 0), new Vector3(0, _minXPosition.y, 0)) < 0.001f || Vector3.Distance(new Vector3(0, _transform.position.y, 0), new Vector3(0, _maxXPosition.y, 0)) < 0.001f)
        {
            _currentAction = _moveDown;
            _moveVertical.SwapDirection();
        }
        else if(Vector3.Distance(new Vector3(_transform.position.x, 0,0), new Vector3(_transform.position.x*_scaleVector.x, 0,0)) < 0.001f)
        {
            _currentAction = _moveVertical;
        }
