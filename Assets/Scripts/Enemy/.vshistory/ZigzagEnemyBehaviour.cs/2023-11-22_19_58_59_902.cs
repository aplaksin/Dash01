...
public class ZigzagEnemyBehaviour : IEnemyBeheviour
{
    ...
    {
        if(_currentAction != _moveDown && (Vector3.Distance(new Vector3(_transform.position.x, 0, 0), new Vector3(_minXPosition.x, 0, 0)) < 0.001f || Vector3.Distance(new Vector3(_transform.position.x, 0, 0), new Vector3(_maxXPosition.x, 0, 0)) < 0.001f))
        {
            _currentAction = _moveDown;
            _lastYP = _transform.position.y;
            _moveVertical.SwapDirection();
        }
        else if(_currentAction != _moveVertical && (Vector3.Distance(new Vector3(0, _lastYP, 0), new Vector3(0, _lastYP + _transform.localScale.y,0)) < 0.001f))
        {
            _currentAction = _moveVertical;
        }
