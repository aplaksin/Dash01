...
public class ZigzagEnemyBehaviour : IEnemyBeheviour
{
    ...
    public void GoExecute(float deltaTime)
    {
        if(_currentAction != _moveDown &&
            (Vector3.Distance(new Vector3(_transform.position.x, 0, 0), new Vector3(_minXPosition.x, 0, 0)) < 0.015f 
            || Vector3.Distance(new Vector3(_transform.position.x, 0, 0), new Vector3(_maxXPosition.x, 0, 0)) < 0.015f))
        {
