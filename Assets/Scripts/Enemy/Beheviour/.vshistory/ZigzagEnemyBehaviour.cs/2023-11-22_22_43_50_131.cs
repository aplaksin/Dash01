...
/*        if(_currentAction == _moveVertical &&
        ...
        if(_currentAction == _moveVertical)
        {
            ...
            if(Vector3.Distance(new Vector3(_transform.position.x, 0, 0), new Vector3(_minXPosition.x, 0, 0)) < 0.02f)
            {
            ...
            if(Vector3.Distance(new Vector3(_transform.position.x, 0, 0), new Vector3(_maxXPosition.x, 0, 0)) < 0.02f))
            {
        ...
        if (_currentAction == _moveDown && (_transform.position.y < (_lastYP - 2 * _transform.localScale.y)))
        {
            _currentAction = _moveVertical;
            //_lastYP = -10f;
        }
