...
//if(Vector3.Distance(new Vector3(_transform.position.x, 0, 0), new Vector3(_maxXPosition.x, 0, 0)) < 0.1f)
            if ( _maxXPosition.x - _transform.position.x < 0.02f) 
            {
            ...
            if(changeAction)
            {
                Debug.Log("changeAction _currentAction = _moveDown;");
                _currentAction = _moveDown;
        ...
        } else if (_currentActionType == EnemyActionType.MoveDown && _lastYP != -100f && (Vector3.Distance(new Vector3(0, _transform.position.y, 0), new Vector3(0, _lastYP - 2 * _transform.localScale.y, 0)) < 0.07f))
        {
            Debug.Log("changeAction _currentAction = _moveVertical;");
            _currentAction = _moveVertical;
