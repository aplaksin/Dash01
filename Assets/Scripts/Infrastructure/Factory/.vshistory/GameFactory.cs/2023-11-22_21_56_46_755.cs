...
public class GameFactory : IGameFactory
{
    ...
    public Enemy CreateEnemy(Vector2 spawnPoint, EnemyType enemyType, GameStageStaticData stage)
    {
        ...
        if(countss < 6 && enemyType == EnemyType.ZigZag)
        {
            //countss++;
            List<Vector3> list = _cellPositionByCoords.Values.ToList();
            enemy.EnemyBeheviour = new ZigzagEnemyBehaviour(enemy.transform, list[0], list[list.Count-1], _scaleVector, enemy._moveSpeed);
        }
