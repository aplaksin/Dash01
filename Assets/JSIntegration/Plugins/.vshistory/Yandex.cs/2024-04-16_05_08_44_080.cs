...
public class Yandex : MonoBehaviour
{
    ...
    [DllImport("__Internal")]
    private static extern void SetPlayerBestScoreExternal(string score);
    ...
    private void SetIsPlayerAuthorised(string isAuthorised)
    {
        ...
        if (Game.GameContext.IsPlayerAuthorised)
        {
            try
            {
                Debug.Log("GetPlayerBestScoreExternal ");
    ...
    private void GetPlayerBestScore(int score)
    {
        Debug.Log("GetPlayerBestScore");
        //int bestS0core = Int32.Parse(score);
        int bestScore = score;
        Debug.Log("score " + score);
        Game.GameContext.PlayerBestScore = bestScore / Game.GameContext.ScoreUIMultiplier;
        Debug.Log("bestScore / Game.GameContext.ScoreUIMultiplier " + Game.GameContext.PlayerBestScore);
        if (bestScore != 0)
        {
    ...
    private void TryToSaveBestScoreYandex()
    {
        ...
        Debug.Log("Game.GameContext.Score " + Game.GameContext.Score);
        Debug.Log("Game.GameContext.PlayerBestScore " + Game.GameContext.PlayerBestScore);
        ...
        if (Game.GameContext.IsBestScoreChanged) { 
            if (Game.GameContext.IsPlayerAuthorised)
            {
                ...
                Debug.Log("SetPlayerBestScoreExternal scoreForSave " + scoreForSave);
                SetPlayerBestScoreExternal(scoreForSave.ToString());
